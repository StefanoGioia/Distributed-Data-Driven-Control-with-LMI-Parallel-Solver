...
%DISTRIBUTED IDENTIFICATION - SAME T RUNS
...
iterTOT=1;  %1 for same inp length but different
incrT=0;
marg = 2 +incrT*(iterTOT-1); %10 increment
...
U0Tfix= (-0.5+rand(m,T)).*10.^(-5+ 10*rand(m,T));
...
%for same inp length but different
 maxErrRel_ND(iter1)= max(max(abs((BA_ND_noZeros-BA_real_noZeros)./BA_real_noZeros)));
...
%for same inp length but different
maxErrRel_ID(iter1)= max(max(abs((BA_ID_noZeros-BA_real_noZeros)./BA_real_noZeros)));
ccc(iter1)=consideredT;
maxrapp(iter1)=maxErrRel_ND(iter1)/maxErrRel_ID(iter1); %max?
...
figure()
plot(1:1:20,log10(maxErrRel_ID)) 
hold on
xlabel('Run')
hold on
